...
public class AddressModelAssembler implements RepresentationModelAssembler<Address, EntityModel<Address>> {
    @Override
    public @NotNull EntityModel<Address> toModel(@NotNull Address entity) {
    ...
    public CollectionModel<EntityModel<Address>> toCollection(
            List<EntityModel<Address>> entities) {
        return CollectionModel.of(entities,
                linkTo(methodOn(AddressController.class).selectAll()).withSelfRel());
    }
}